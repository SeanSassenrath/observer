...
const MeditationPlayer = ({ route }: MeditationPlayerStackScreenProps<'MeditationPlayer'>) => {
  ...
  useEffect(() => {
    ...
    loadingListener = SoundPlayer.addEventListener('FinishedLoading', ({ success }) => {
      console.log('finished loading >>>', success);
      getInfo();
    })
    console.log('URL >>>', meditation.url);
    SoundPlayer.playUrl(meditation.url);
    // setTrack();
    // addTracks();
  ...
  const playSong = async () => {
    ...
    // console.log('MEDITATION URL:', meditation.url);
    // console.log('WHAT?')
    // const status = await playbackObject.loadAsync({
    //   uri: meditation.url,
    //   // uri: 'file:///private/var/mobile/Containers/Data/Application/EE2D57BF-5402-406A-9D4B-E3E5D2E2FCEC/tmp/org.reactjs.native.example.Observer-Inbox/03-Meditation-Tuning_In_to_New_Potentials.mp3'
    // },
    //   { shouldPlay: true }
    // )
