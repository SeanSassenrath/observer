...
export function calculateNameSimilarity(
  ...
  const sourceNormalized = sourceFileName
  ...
  // 1. Exact match check
  if (
    userNormalized === meditationNormalized ||
  ...
  const updateScore = calculateUpdateScore(
    userNormalized,
    meditationNormalized,
  );
  console.log(`  Update score: ${(updateScore * 100).toFixed(1)}%`);

  // 5. Exact keyword matching
  const keywordScore = calculateExactKeywordScore(
    userNormalized,
    meditationNormalized,
  );
  ...
  const fuzzyScore = calculateTraditionalFuzzy(
    userNormalized,
    meditationNormalized,
    sourceNormalized,
  );
...
function expandSeriesAbbreviations(text: string): string {
  ...
  for (const series of DR_JOE_SERIES) {
    ...
    if (abbrevPattern.test(expanded)) {
      ...
      console.log(
        `    🔄 Expanded "${series.abbreviation}" → "${series.fullName}"`,
      );
    }
    ...
    for (const userNum of userNumbers) {
      for (const medNum of meditationNumbers) {
        if (numbersMatch(userNum, medNum)) {
          ...
          const seriesNameScore = calculateEditDistanceSimilarity(
            userWithoutNumbers,
            meditationWithoutNumbers,
          );

          if (seriesNameScore > 0.7) {
            console.log(
              `    📊 Series match: "${userNum}" = "${medNum}", series similarity: ${(seriesNameScore * 100).toFixed(1)}%`,
            );
...
function extractNumbers(text: string): string[] {
  ...
  for (const word of tokens) {
    ...
    // Check for part/session/chapter patterns
      ...
      word.includes('chapter')
    ) {
      const match = word.match(
        /(part|session|chapter)\s*(\d+|[ivxlcdm]+|one|two|three|four|five|six|seven|eight|nine|ten)/,
      );
      if (match) {
...
function numbersMatch(num1: string, num2: string): boolean {
  ...
  for (const [canonical, variations] of Object.entries(NUMBER_VARIATIONS)) {
    if (
      variations.some(v => v.includes(num1.toLowerCase())) &&
 ...
 */
function calculateUpdateScore(
  userText: string,
  meditationText: string,
): number {
  const hasUserUpdate = UPDATE_INDICATORS.some(indicator =>
    userText.includes(indicator),
  );
  const hasMeditationUpdate = UPDATE_INDICATORS.some(indicator =>
    meditationText.includes(indicator),
  );
  ...
  if (coreScore > 0.8) {
    if (hasUserUpdate && hasMeditationUpdate) {
      ...
      console.log(
        `    🔄 PERFECT: Both have update indicators, core similarity: ${(coreScore * 100).toFixed(1)}%`,
      );
      ...
      console.log(
        `    🔄 MISMATCH: User wants updated, meditation is original, core similarity: ${(coreScore * 100).toFixed(1)}%`,
      );
      ...
      console.log(
        `    🔄 PARTIAL: User has original, meditation is updated, core similarity: ${(coreScore * 100).toFixed(1)}%`,
      );
      ...
      console.log(
        `    🔄 ORIGINAL: Both are original versions, core similarity: ${(coreScore * 100).toFixed(1)}%`,
      );
 ...
 */
function calculateExactKeywordScore(
  userText: string,
  meditationText: string,
): number {
  ...
  for (const keyword of CRITICAL_KEYWORDS) {
    ...
    const meditationHasKeyword = keywordParts.every(part =>
      meditationWords.includes(part),
    );
  ...
  if (totalKeywordsInUser > 0) {
    ...
    if (keywordMatchRate === 1.0) {
      ...
      const meditationWithoutKeywords = removeKeywords(
        meditationText,
        CRITICAL_KEYWORDS,
      );
      const baseSimilarity = calculateEditDistanceSimilarity(
        userWithoutKeywords,
        meditationWithoutKeywords,
      );

      if (baseSimilarity > 0.7) {
        console.log(
          `    🎯 KEYWORD MATCH: All ${totalKeywordsInUser} keywords matched, base similarity: ${(baseSimilarity * 100).toFixed(1)}%`,
        );
      ...
      console.log(
        `    ⚠️ KEYWORD MISMATCH: Only ${matchedKeywords}/${totalKeywordsInUser} keywords matched`,
      );
 ...
 */
function calculateTraditionalFuzzy(
  userText: string,
  meditationText: string,
  sourceText: string,
): number {
  ...
  const wordSimilarity = Math.max(
    ...
    sourceWords.length > 0
      ? calculateWordOverlapSimilarity(userWords, sourceWords)
      : 0,
  );
 ...
 */
function calculateWordOverlapSimilarity(
  words1: string[],
  words2: string[],
): number {
